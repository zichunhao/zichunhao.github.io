...
I built the [Gymnasium](https://gymnasium.farama.org/) environment wrapping the physics simulator, the shaped reward design, the graded ladder of target states, and the baseline policies students must beat. Students implement REINFORCE from scratch and can compare it against PPO, SAC, and TQC through [Stable-Baselines3](https://stable-baselines3.readthedocs.io/), across MLP, LSTM, and Transformer policies. The project also covers experimental rigor through multi-seed evaluation, reward ablations, and a transfer learning recipe that pretrains on an easier target before fine-tuning on the hardest non-Gaussian cat state.
